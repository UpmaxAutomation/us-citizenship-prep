...
export default function StudyClientEs() {
        ...
        <div className="fixed inset-0 z-[90] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-md w-full space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-medium">Configuracion</h2>
              <button
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-lg font-bold tracking-tight">
                <span className="text-blue-400">EE.UU.</span> Ciudadania Prep
              </h1>
              <p className="text-xs text-slate-500 mt-0.5">
                128 Preguntas Civicas &middot; Examen 2025
              </p>
            </div>
        ...
        <div className="flex gap-1 bg-slate-900/50 p-1 rounded-xl border border-slate-800/50">
          {(
            [
              ...
              { key: "quiz", label: "Examen", icon: "✍️" },
              { key: "dashboard", label: "Progreso", icon: "📊" },
            ] as const
          ).map((tab) => (
            <button
              key={tab.key}
              ...
              <div className="text-center py-16 text-slate-500">
                ...
                <p className="text-lg font-medium text-slate-300">
                  {studyFilter === "mastered"
                    ? "Aun no hay preguntas dominadas"
                    : studyFilter === "review"
                      ? "No hay preguntas para repasar ahora!"
                      : studyFilter === "new"
                        ? "Ya has visto todas las preguntas!"
                        : "No se encontraron preguntas"}
                </p>
                <p className="text-sm mt-2">
                  {studyFilter !== "all" &&
                    "Intenta cambiar el filtro de arriba."}
                </p>
              </div>
            )}
          </div>
        ...
        {mode === "voice" && (
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between bg-slate-900/50 border border-slate-800 rounded-xl px-4 py-3">
              <div>
                <p className="text-sm font-medium text-white">Modo Automatico</p>
                <p className="text-xs text-slate-500">
                  Lee la pregunta, escucha y avanza automaticamente
                </p>
              </div>
              <button
            ...
            {filteredQuestions.length > 0 && currentQuestion ? (
              ...
              <div className="text-center py-16 text-slate-500">
                ...
                <p className="text-sm mt-2">
                  Intenta cambiar el filtro de categoria.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 mb-4">
              ...
              <div className="flex items-center gap-4">
                <label className="text-sm text-slate-400">Preguntas:</label>
                <div className="flex gap-2">
                  {[10, 20, 50, 128].map((n) => (
                    <button
                      key={n}
              ...
              <p className="text-xs text-slate-500 mt-3">
                Durante el examen real USCIS 2025, un oficial hace hasta 20
                preguntas. Necesitas al menos 12 correctas (60%) para pasar.
              </p>
            </div>
            ...
            <div className="text-center">
              <button
                onClick={() => {
                  if (
                    window.confirm(
                      "Estas seguro de que quieres reiniciar todo el progreso? Esto no se puede deshacer."
                    )
                  ) {
                    resetProgress();
                  }
