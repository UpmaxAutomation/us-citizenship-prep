...
import { questionsTr as questions } from "@/app/data/questions-tr";
...
function generateOptions(
  ...
  const otherAnswers = allQuestions
    ...
    .filter(
      (a) =>
        !correctAnswers.includes(a) && !a.startsWith("Cevaplar degisecektir")
    );
...
const categoryBadge: Record<string, string> = {
  "Amerikan Hukumeti": "bg-blue-500/15 text-blue-300 border-blue-500/30",
  "Amerikan Tarihi": "bg-orange-500/15 text-orange-300 border-orange-500/30",
...
export default function PracticeTestTr() {
    ...
    return (
      <div className="w-full max-w-2xl mx-auto">
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-8">
          <h2 className="text-2xl font-bold mb-4">Deneme Sinavi</h2>
          <div className="space-y-3 text-slate-400 leading-relaxed">
            <div className="flex items-start gap-3">
              ...
              <p>
                Resmi USCIS 128 sivil bilgiler sorusu banksindan rastgele
                sorular, gercek naturalizasyon mulakati gibi.
              </p>
            </div>
            <div className="flex items-start gap-3">
              ...
              <p>
                Gecmek icin gereken dogru cevap sayisi. 12 dogru cevaba
                ulastiginizda sinav durur — USCIS yetkilisinin gectiginizde
                sormaya son vermesi gibi.
              </p>
            </div>
            <div className="flex items-start gap-3">
              ...
              <p>
                {stateInfo
                  ? `Eyalete ozel sorular ${stateInfo.name} icin kisisellestirildi.`
                  : "Eyalete ozel sorulari (senatorler, vali, baskent) kisisellestirilmek icin ayarlardan eyaletinizi secin."}
              </p>
            </div>
          </div>
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-2">
            <span className="text-sm text-slate-400">
              Soru {currentIndex + 1} / {testQuestions.length}
            </span>
          ...
          !isCorrectAnswer(selectedAnswer) && (
            <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-xl p-4 mb-6">
              <p className="text-sm text-emerald-400">
                Dogru cevap{currentQuestion.answers.length > 1 ? "lar" : ""}:{" "}
                <span className="font-medium">
                  {currentQuestion.answers.join(" / ")}
                </span>
              </p>
              {currentQuestion.answerPhonetics && currentQuestion.answerPhonetics.length > 0 && (
                <p aria-hidden="true" className="text-xs italic text-emerald-400/50 mt-1.5">
                  Okunus: {currentQuestion.answerPhonetics.join(" / ")}
                </p>
              )}
            </div>
          )}
    ...
    <div className="w-full max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-10 text-center">
        ...
        <h2 className="text-2xl font-bold mt-6">
          {passed ? "Gectin!" : "Calismaya Devam Et!"}
        </h2>

        {passed ? (
          <p className="text-slate-400 mt-2 text-sm leading-relaxed max-w-md mx-auto">
            {totalAnswered} sorudan {correctCount} tanesini{" "}
            {formatTime(elapsedSeconds)} icinde dogru cevaladiniz. 12/20 gecme
            esigini astisiniz — harika is!
          </p>
        ) : (
          <p className="text-slate-400 mt-2 text-sm leading-relaxed max-w-md mx-auto">
            {totalAnswered} sorudan {correctCount} tanesini{" "}
            {formatTime(elapsedSeconds)} icinde dogru cevapladiniz. Gecmek icin
            12 dogru cevap gerekiyor. Endiselenmeyin — tekrar deneyin!
          </p>
        )}
        ...
        <div className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-800/50 border border-slate-700/50 text-sm">
          ...
          <span className="text-slate-500">gecen sure</span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mt-8">
          ...
          <ShareButton
            title="Vatandaslik Deneme Sinavi Sonuclari"
            text={`ABD vatandaslik deneme sinavinda ${correctCount}/${results.length} aldim!`}
            url="https://www.uscitizenshiptestprep.com/tr/deneme-sinavi"
      ...
      {missedQuestions.length > 0 && (
        <details className="group rounded-xl bg-slate-900/50 border border-slate-800/50 overflow-hidden">
          <summary className="flex items-center justify-between cursor-pointer px-6 py-4 text-white font-medium hover:bg-slate-800/30 transition-colors list-none [&::-webkit-details-marker]:hidden">
            <span className="pr-4">
              Yanlis Cevaplari Gozden Gecir ({missedQuestions.length})
            </span>
            <svg
              width="20"
              height="20"
            ...
            {missedQuestions.map((r) => (
              ...
              >
                <p className="text-white font-medium text-sm">
                  S{r.questionId}. {r.question}
                </p>
                <div className="mt-2 space-y-1">
                  <p className="text-red-400 text-sm">
                    Sizin cevabisiniz: {r.selectedAnswer}
                  </p>
                  <p className="text-emerald-400 text-sm">
                    Dogru cevap: {r.correctAnswers.join(" / ")}
                  </p>
                  {r.answerPhonetics && r.answerPhonetics.length > 0 && (
                    <p aria-hidden="true" className="text-xs italic text-emerald-400/50">
                      Okunus: {r.answerPhonetics.join(" / ")}
                    </p>
                  )}
                </div>
              </div>
